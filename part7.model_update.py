...
for i in range(sectors):
    sector_mins.append(n)
    n += sector_extent
    sector_maxs.append(n)

df_sectors = pd.DataFrame(data = {'ID': sector_ids, 'MIN': sector_mins, 'MAX': sector_maxs})
...
# initialize an empty dataframe that will eventually hold perturbation information for all blocks and shells, for all phases (df_updated_model)
# df_updated_model = pd.DataFrame()
all_shells = df_shells['SHELL#']
all_blocks = df_blocks['BLOCK#']
last_block = all_blocks.iloc[-1]
...
df_variance = pd.DataFrame(columns = ['Layer', 'Iteration', 'total_paths', 'Unexplained_diff_mean', 'Unexplained_diff_std', 'Unexplained_diff_var'])

def backmap_residual(lists_of_paths, list_idx):
    # loop through `lists_of_paths` to determine which phases are present
    df_idx = 0    
    df_block_info = pd.DataFrame(columns = ['SHELL#', 'BLOCK#', 'PHASE', 'PATH_ID', 'START_LAT', 'START_LON', 'START_DEPTH', 'CENTER_LAT', 'CENTER_LON', 'END_LAT', 'END_LON', 'END_DEPTH', 'PATH_LENGTH_DEG', 'PATH_LENGTH_KM', mod_input.perturbation_header, 'AZIMUTH', 'SECTOR', 'COMP_WEIGHT', 'HIPR_STA_WEIGHT', 'HIPR_EQ_WEIGHT'])
    df_residuals = pd.DataFrame(columns = ['PHASE', 'PATH_ID', 'PREM', 'DT', 'STARTING_pred_time', 'STARTING_DT', 'UNEXPLAINED_DIFF']) #read_csv(f'{iteration_directory_path}/residuals.csv')

    for list_of_paths in lists_of_paths:
        phase = str(list_of_paths[0].split('/')[2])
        phase_name = str(phase.split('_')[0])
        # df_data = pd.read_csv(f'./{mod_input.phases_directory}/{phase}/{mod_input.data_directory}/{phase}_{mod_input.crustal_model}_data.csv')
        ...
        for path in list_of_paths:
            ...
            if path_max_depth <= layer_stripping_depth and path_max_depth > layer_depth_min:
                ...
                # grab Hongyu's residual and comprehensive weights for the current path
                dataset_residual = float(df_data.loc[df_data['PATH_ID'] == path_id][mod_input.residual_header])
                dataset_comp_weight = float(df_data.loc[df_data['PATH_ID'] == path_id]['COMPREHENSIVE_WEIGHT'])
                dataset_hipr_station_weight = float(df_data.loc[df_data['PATH_ID'] == path_id]['HIPR_STA_WEIGHT'])
                dataset_hipr_event_weight = float(df_data.loc[df_data['PATH_ID'] == path_id]['HIPR_EQ_WEIGHT'])
                ...
                if backmap == True:
                    ...
                    for segment in range(len(df_p)):
                        ...
                        prem_val = float(df_p[mod_input.segment_property_header].iloc[segment])
                        ...
                        df_p.loc[segment, mod_input.segment_property_header] = seg_v
                    ...
                    df_p_update['v_update'] = df_p_update[f'{mod_input.segment_property_header}'] * x
                    ...
                    df_p_update[mod_input.segment_property_header] = df_p_update['SEG_DIST_KM'] / df_p_update['SEG_TIME']
                    ...
                    for pair in range(len(df_path_elements)):
                        ...
                        # df_shell_file = pd.read_csv(f'{layer_directory}/tmp_block_info_idx_{list_idx}/shell_{shell_no}.csv')
                        # df_block_file = pd.read_csv(f'{layer_directory}/tmp_block_info_idx_{list_idx}/shell_{shell_no}/block_{block_no}.csv')
                        ...
                        updated_velocity = df_slice[mod_input.segment_property_header].iloc[0]
                        mid_depth = df_slice['MID_DEPTH'].iloc[0]
                        prem_velocity = mod_refmodels.prem_vs(mid_depth)
                        ...
                        total_length_deg = df_slice['SEG_DIST_DEG'].sum()
                        start_lat = df_slice['START_LAT'].iloc[0]
                        start_lon = df_slice['START_LON'].iloc[0]
                        start_depth = df_slice['START_DEPTH'].iloc[0]
                        end_lat = df_slice['END_LAT'].iloc[-1]
                        end_lon = df_slice['END_LON'].iloc[-1]
                        end_depth = df_slice['END_DEPTH'].iloc[-1]
                        ...
                        df_block_info.loc[len(df_block_info)] = [shell_no, block_no, phase, path_id, start_lat, start_lon, start_depth, segment_center_coords[0], segment_center_coords[1], end_lat, end_lon, end_depth, total_length_deg, total_length_km, percent_perturbation, azimuth, az_sector, dataset_comp_weight, dataset_hipr_station_weight, dataset_hipr_event_weight]
    ...
    for d_id in range(df_idx):
        ...
        for shell_no in shells_to_update: # all_shells:
...
def model_smoothing(shell_no):
    ...
    for block_no in all_blocks:
        ...
        # ONLY ENTER THE SMOOTHING LOOP if there is more than one segment in df_smoothing.
            ...
            if layer_azimuthal_weighting == True:
                ...
                for path_sectors in range(len(unique_path_segment_sectors_hist)):
                    ...
                    if paths_in_sector > 0:
                        ...
                        # loop throuh each path segment in df_final_smoothing and retrieve their attributes
                        for segment_final in range(len(df_final_smoothing)):
                            segment_final_pert = df_final_smoothing[mod_input.perturbation_header].iloc[segment_final]
                            segment_final_comp = df_final_smoothing['COMP_WEIGHT'].iloc[segment_final]
                            segment_final_gaus = df_final_smoothing['GAUSS_WEIGHT'].iloc[segment_final]
                            segment_final_hipr_sta = df_final_smoothing['HIPR_STA_WEIGHT'].iloc[segment_final]
                            segment_final_hipr_eq = df_final_smoothing['HIPR_EQ_WEIGHT'].iloc[segment_final]
                            segment_final_length = df_final_smoothing['PATH_LENGTH_KM'].iloc[segment_final]

                            if layer_hipr_weighting == True:
                                weights = [segment_final_comp, segment_final_gaus, segment_final_hipr_sta, segment_final_hipr_eq, segment_final_length]
                            elif layer_hipr_weighting == False:
                                weights = [segment_final_comp, segment_final_gaus, segment_final_length]
                            ...
                            if math.isnan(segment_final_pert) == False:
                                pre_az_numerator += (segment_final_pert * np.prod(weights_to_use))
                                pre_az_denominator += (np.prod(weights_to_use))
                            
                            if math.isnan(segment_final_pert) == True or math.isnan(segment_final_comp) == True or math.isnan(segment_final_gaus) == True or math.isnan(segment_final_length) == True:
                                with open(f'{iteration_directory_path}/nan_error_file_shell_{shell_no}.txt', 'a') as nan_file:
                                    nan_file.write(f'///// ======== /////\n')
                                    nan_file.write(f'Block: {block_no}\n')
                                    nan_file.write(f'Segment perturbation: {segment_final_pert}\n')
                                    nan_file.write(f'Segment comprehensive weight: {segment_final_comp}\n')
                                    nan_file.write(f'Segment Gaussian weight: {segment_final_gaus}\n')
                                    nan_file.write(f'Segment length: {segment_final_length}\n')
                                    nan_file.write(f'HIPR station weight: {segment_final_hipr_sta}\n')
                                    nan_file.write(f'HIPR event weight: {segment_final_hipr_eq}\n')
                            
                            if (segment_final_gaus == 0.) or (segment_final_comp == 0.) or (segment_final_length == 0.) or (segment_final_hipr_sta == 0.): # or (segment_final_hipr_eq == 0.):
                                with open(f'{iteration_directory_path}/zero_denom_error_file_shell_{shell_no}.txt', 'a') as zero_denom_error_file:
                                    zero_denom_error_file.write(f'///// ======== /////\n')
                                    zero_denom_error_file.write(f'Block: {block_no}\n')
                                    zero_denom_error_file.write(f'Segment perturbation: {segment_final_pert}\n')
                                    zero_denom_error_file.write(f'Segment comprehensive weight: {segment_final_comp}\n')
                                    zero_denom_error_file.write(f'Segment Gaussian weight: {segment_final_gaus}\n')
                                    zero_denom_error_file.write(f'Segment length: {segment_final_length}\n')
                                    zero_denom_error_file.write(f'HIPR station weight: {segment_final_hipr_sta}\n')
                                    zero_denom_error_file.write(f'HIPR event weight: {segment_final_hipr_eq}\n')
                ...
                # ideally, this is where i would like to add in the overall gaussian weight.
                for az_sector_average in azimuthal_sector_averages:
                    mean_element_perturbation += (1. / azimuthal_denominator) * az_sector_average

            # if the azimuthal weighting option is turned off, just compute a weighted average based on the...
            # path segment length, the gaussian weight, and the comprehensive weight
                ...
                for segment_final in range(len(df_smoothing)):
                    segment_final_pert = df_smoothing[mod_input.perturbation_header].iloc[segment_final]
                    segment_final_comp = df_smoothing['COMP_WEIGHT'].iloc[segment_final]
                    segment_final_gaus = df_smoothing['GAUSS_WEIGHT'].iloc[segment_final]
                    segment_final_hipr_sta = df_smoothing['HIPR_STA_WEIGHT'].iloc[segment_final]
                    segment_final_hipr_eq = df_smoothing['HIPR_EQ_WEIGHT'].iloc[segment_final]
                    segment_final_length = df_smoothing['PATH_LENGTH_KM'].iloc[segment_final]
                    ...
                    if math.isnan(segment_final_pert) == False:
                        smoothing_numerator += (segment_final_pert * np.prod(weights_to_use))
                        smoothing_denominator += (np.prod(weights_to_use))
                    
                    # if segment_final_gaus * segment_final_comp * segment_final_length == 0.:
                    #     pass
                    # else:
                    #     smoothing_numerator += (segment_final_pert * segment_final_comp * segment_final_gaus * segment_final_length)
                    #     smoothing_denominator += (segment_final_gaus * segment_final_comp * segment_final_length)
...
def find_smoothing_radii(shell_no):
    ...
    for block_no in all_blocks:
        ...
        for radius in layer_smoothing_radii:
            ...
            # if there aren't, keep sufficient_coverage equal to False and check the next radius
        ...
        # df_smoothing = df_smoothing[['NEIGHBOR', 'PHASE', 'PATH_ID', 'GAUSS_WEIGHT']]
...
for layer_bottom_shell in mod_input.layer_stripping_base_shells:
    layer_start = time.time()
    layer_top_shell = mod_input.layer_stripping_top_shells[layers_complete]
    ...
    layer_proportion_of_dataset_to_use = mod_input.proportion_of_dataset_to_use[layers_complete]
    layer_multiprocessing_path_increments = mod_input.multiprocessing_path_increments[layers_complete]
    ...
    layer_hipr_weighting = mod_input.hipr_weighting[layers_complete]
    ...
    print('PARTITIONING PATHS INTO BATCHES FOR BACKMAPPING')
    paths_lists = []
    paths_list = []
    ...
    for phase in phases_to_update:
        ...
        # paths = glob.glob(f'./{mod_input.phases_directory}/{phase}/{mod_input.resampled_paths_directory}/{phase_name}_1?_*.csv') # delete
    ...
    # determine the total approximate number of paths that should go in each main list (`divided_elememts`) and the number of divisions of the total paths list there will be (`divisions`)
    if tot_paths > layer_multiprocessing_path_increments:
        divisions = math.ceil(tot_paths / layer_multiprocessing_path_increments)
        divided_elements = tot_paths / divisions
        
        start_path_idx = 0
        end_path_idx = int(divided_elements)
        
        # loop through each division of the main dataset and slice the total list of paths accordingly
        for division in range(1, divisions + 1):
            if division == divisions:
                path_list = paths_list[start_path_idx:]
                
            else:
                path_list = paths_list[start_path_idx:end_path_idx]
            ...
            start_path_idx += int(divided_elements)
            end_path_idx += (int(divided_elements))

    else:
        paths_lists.append(paths_list)
    ...
    for list_index in range(len(paths_lists)):
        try:
            print(f'MAKING TEMPORARY BLOCK INFO FILES: batch {list_index + 1} of {len(paths_lists)}')
    ...
    while continue_layer_iterating == True:
        ...
        print()
        print(f'*****')
        print(f'***** BEGIN ITERATION #{layer_iteration} FOR LAYER #{layers_complete} of {len(mod_input.layer_stripping_base_shells)}, DEPTH RANGE: {layer_depth_min} km - {layer_stripping_depth} km *****')
        print(f'*****')
        ...
        for phase in phases_to_update:
            ...
            try:
                os.mkdir(f'./{mod_input.phases_directory}/{phase}/{mod_input.backmapped_paths_directory}_{job_id}') # modify_sol: scratch?
            except:
                pass
        ...
        ############################################
        print(f'START BACKMAPPING & PATH-TO-BLOCK FORMAT')
        ...
        if __name__ == '__main__':
            ...
            for list_index in range(len(paths_lists)):
                process_idx += 1
                print(f'  - starting process {process_idx} of {len(paths_lists)}')                    
        ...
        print(f'FINISHED BACKMAPPING & PATH-TO-BLOCK FORMAT; runtime: {backmapping_time / 60} minutes / {(backmapping_time / 60) / 60} hours')

        
        ## Merge all files for the different processes into the main block files:
        print(f'START MERGING TMP BLOCK FILES INTO MAIN BLOCK FILES')
        ...
        if __name__ == '__main__':
            ...
            for shell_no in shells_to_update:
                print(f'  - starting process for shell {shell_no}; total of {len(shells_to_update)} shells to merge')
        ...
        print(f'FINISHED MERGING TMP BLOCK FILES INTO MAIN BLOCK FILES; runtime: {merge_time} seconds / {merge_time / 60} minutes')
        ...
        unexplained_diffs = df_residuals['UNEXPLAINED_DIFF'].to_numpy()
        ...
        if layer_iteration > 1:
            ...
            print(f'ITERATION #{layer_iteration}; variance: {var1}, variance difference: {var_diff}')
        residual_time = time.time() - residual_start

        print(f'FINISHED COMPUTING VARIANCE: {residual_time} seconds')
        
        print('-----------------------------------------')
        print('-  backmapping COMPLETE for all phases  -')
        ...
        if layer_iteration == 1: # modify/uncomment
            print(f'START COMPUTATION OF SMOOTHING RADII (FIRST LAYER ITERATION)')
            ...
            if __name__ == '__main__':
                ...
                for shell_no in shells_to_update:
                    print(f'  - starting process for shell {shell_no}; total: {len(shells_to_update)} shells to update')
                    try:
            ...
            print(f'FINISHED FINDING SMOOTHING RADII, runtime: {smoothing_radii_time / 60} minutes / {(smoothing_radii_time / 60) / 60} hours')
            

        else:
            print('START UPDATING SMOOTHING BLOCKS')
            smoothing_radii_start = time.time()
            ...
            smoothing_radii_time = time.time() - smoothing_radii_start
            print(f'FINISHED UPDATING SMOOTHING BLOCKS:, runtime: {smoothing_radii_time / 60} minutes / {(smoothing_radii_time / 60) / 60} hours')
        ...
        print('START MODEL SMOOTHING')
        smoothing_start = time.time()
        if __name__ == '__main__':
            smoothing_process_list = []
            for shell_no in all_shells:
                if shell_no in shells_to_update:
                    print(f'  - starting process for shell {shell_no}; total of {len(shells_to_update)} to smooth')
        ...
        print(f'FINISHED MODEL SMOOTHING; runtime: {smoothing_time} seconds / {smoothing_time / 60} minutes')

        
        df_model = df_model.drop(df_model.index)
        # df_model = pd.DataFrame(columns = df_model.columns)
        for shell_no in all_shells:
        ...
        print('resetting block info files, df_rms, and df_model')
        rms_values = []
        ...
        with open(f'{iteration_directory_path}/{model}_update_info.txt', 'w') as fout:
            ...
            fout.write(f'mean time for smoothing per block: {(smoothing_time / len(shells_to_update)) / 10312} seconds\n')
            fout.write(f'mean unexplained difference (dataset residual - predicted residual): {unexplained_means[-1]} seconds \n')
            fout.write(f'unexplained differences std: {unexplained_stds[-1]}\n')
            fout.write(f'unexplained differences variance: {unexplained_vars[-1]}\n')
    ...
    # shutil.rmtree(f'{layer_directory}/smoothing_block_information')
    
    df_layer_variance = pd.DataFrame(data = {'Layer': layers_complete, 'Iteration': list(range(1, len(unexplained_means) + 1, 1)), 'total_paths': total_paths_itr, 'Unexplained_diff_mean': unexplained_means, 'Unexplained_diff_std': unexplained_stds, 'Unexplained_diff_var': unexplained_vars})
